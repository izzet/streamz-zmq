...
async def pub_sub_example():
    ...
    publisher_thread = threading.Thread(
        target=zmq_publisher_worker,
        args=(port, weather_data, 0.2, zmq.PUB, "WEATHER ")
    )
    
    # Subscriber: Listen for NYC weather only
    nyc_weather = []
    nyc_subscriber = Stream.from_zmq(f"tcp://localhost:{port}", sock_type=zmq.SUB, subscribe=b"WEATHER NYC")
    (nyc_subscriber
     .map(lambda x: x.decode('utf-8') if isinstance(x, bytes) else str(x))
     .sink(lambda x: nyc_weather.append(x)))
    
    # Subscriber: Listen for all weather
    all_weather = []
    all_subscriber = Stream.from_zmq(f"tcp://localhost:{port}", sock_type=zmq.SUB, subscribe=b"WEATHER")
    (all_subscriber
     .map(lambda x: x.decode('utf-8') if isinstance(x, bytes) else str(x))
...
async def push_pull_example():
    ...
    producer_thread = threading.Thread(
        target=zmq_publisher_worker,
        args=(port, tasks, 0.15, zmq.PUSH, "WORK ")
    )
...
async def pipeline_example():
    ...
    stage1_thread = threading.Thread(
        target=zmq_publisher_worker,
        args=(stage1_port, raw_data, 0.2, zmq.PUSH)
    )
